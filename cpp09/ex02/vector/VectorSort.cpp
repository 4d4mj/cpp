...
std::vector<VectorElement> wrapVectorWithIndices(const std::vector<int> &data)
{
    ...
    for (std::size_t i = 0; i < data.size(); ++i)
        elements.push_back(VectorElement(data[i], i));
                                ...
                                int &compCount)
{
    ...
    {
        if (compareVectorElements(data[i], data[i + 1], compCount))
        {
...
{
    std::vector<VectorElement> reorderedPend;
    reorderedPend.reserve(sortedMain.size());
                              ...
                              int &compCount)
{
    ...
    for (std::size_t i = 0; i < insertionOrder.size(); ++i)
    {
        ...
        {
            int insertPos = binarySearchVector(main, pend[pendIndex - 1], high - 1, compCount);
    ...
    int &compCount)
{
    // Base cases
    if (data.empty())
        return std::vector<VectorElement>();
    ...
    if (data.size() == 2)
    {
        std::vector<VectorElement> result = data;
        if (compareVectorElements(result[0], result[1], compCount))
            std::swap(result[0], result[1]);
        return result;
    }

    // Pair and sort
    std::vector<VectorElement> main, pend;
    pairAndSortVectorElements(data, main, pend, compCount);
    ...
    insertVectorPendIntoMain(sortedMain, reorderedPend, compCount);
